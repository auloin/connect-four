...
class AdaptedGame(Connect4Game):

    def __init__(self, rows=6, cols=7):
        super().__init__(rows=rows, cols=cols)

    def get_board_copy(self):
        return [[self._board[j][i] for i in range(self._rows)] for j in range(self._cols)]

    def get_legal_actions(self, board=None):
        actions = []
        
        for a in range(self._cols):
            if self.is_legal(a, board):
                actions.append(a)

        return actions

    def is_legal(self, a, board):
        board = board if board else self._board
        return board[a][self._rows - 1] == 0

    def simulate(self, a):
        for r in range(self._rows):
            if self._board[a][r] == 0:
                board = self.get_board_copy()
                board[a][r] = self._turn
                return board, self.check_board_state(board, (a, r))
    ...
    def check_board_state(self, board, pos):
        """
        Checks for win/draw from newly added disc
        :param pos: position from which to check the win
        :return: player number if a win occurs, 0 if a draw occurs, None otherwise
